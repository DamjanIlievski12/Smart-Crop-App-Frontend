  ...
  useEffect(() => {
    ...
    async function analyze() {
      ...
      try {
        ...
        const conditions: Condition[] = (data.conditions ?? []).map((c) => {
                ...
                {analysis.diseaseRisks.map(({ label, level, pct, bar }) => (
                  <div key={label}>
                    <div className="flex justify-between text-xs mb-1">
                      ...
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full font-medium ${level === "High" ? "bg-red-100 text-red-600" : level === "Medium" ? "bg-yellow-100 text-yellow-600" : "bg-green-100 text-green-600"}`}
                      >
                        {level}
                      </span>
                    </div>
