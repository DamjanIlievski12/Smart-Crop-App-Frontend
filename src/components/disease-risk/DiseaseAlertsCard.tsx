...
const severityBarStyles: Record<SeverityLevel, string> = {
  Low: 'bg-yellow-400',
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 p-5 mb-4">
      ...
      <div className="flex flex-col gap-4">
        {alerts.length === 0 && (
          <p className="text-sm text-gray-400 text-center py-6">No disease alerts for this field.</p>
        )}
        {alerts.map((disease) => {
          return (
            <div key={disease.name} className="border border-gray-100 rounded-xl p-5">
              ...
              <div>
                ...
                <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                  <div
                    className={`h-full rounded-full transition-all ${severityBarStyles[disease.severity]}`}
                    style={{ width: `${Math.max(0, Math.min(100, disease.probability))}%` }}
                  />
                </div>
              </div>
            </div>
