...
function extractMetricDisplay(value: string, level?: string): { valueText: string; levelText: string | null } {
...
export default function CurrentWeatherCard({ metrics, temp, condition, location, lastUpdated }: CurrentWeatherCardProps): React.ReactElement {
  return (
    <div className="rounded-2xl bg-gradient-to-r from-[#3b82f6] to-[#2563eb] p-6 mb-4 flex gap-6">
      <div className="flex-1 flex flex-col justify-center">
        ...
        <div className="flex items-center gap-5 mb-5">
          ...
          <div>
            <p className="text-5xl font-bold text-white leading-none">{temp != null ? `${temp}°C` : '—'}</p>
            <p className="text-base text-white/90 mt-2">{condition ?? '—'}</p>
          </div>
        </div>
        <p className="text-sm text-white font-medium">{location ?? '—'}</p>
        {lastUpdated && <p className="text-xs text-white/70 mt-1">Last updated: {lastUpdated}</p>}
      </div>

      <div className="grid grid-cols-2 gap-3 flex-1">
        {metrics.map(({ label, value, level, icon: Icon }) => {
          const { valueText, levelText } = extractMetricDisplay(value, level);
          const badgeClasses = levelText ? levelBadgeClasses[levelText.toLowerCase()] ?? levelBadgeClasses.unknown : null;
          ...
          return (
            <div key={label} className="bg-white/15 backdrop-blur-sm rounded-xl p-5 flex flex-col justify-center">
              ...
              {levelText && (
                <span className={`mt-2 inline-flex w-fit items-center rounded-full px-2.5 py-0.5 text-xs font-semibold ${badgeClasses}`}>
                  {levelText}
                </span>
              )}
            </div>
